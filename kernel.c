...
// 在 kernel.c 的最顶端（#define 的下方，任何函数之前）加上这一行：
void print_hex(uint32_t val);
extern void probe_ext2_partition(void);
...
extern uint32_t resolve_path(const char *path);
...
void c_syscall_handler(struct syscall_regs *regs) {
  ...
  switch (syscall_num) {
    ...
    case 2: {
            ...
            // 在 kernel.c 的 c_syscall_handler 中
...
// 定义一个静态全局用户栈，防止每次运行时栈被污染
__attribute__((aligned(16))) static uint8_t user_stack[4096];
